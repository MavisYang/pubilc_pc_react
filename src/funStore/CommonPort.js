...
/** 系统状态判断 status 1代表服务正常 2代表服务维护API_URL.appStatus */
export const AppStatus = () => {
    return AxiosCore.request({url: API_URL.appStatus, method: 'get', isAuth: false})
}
/**获取验证码 **/
...
/** 判断补全群宠用户信息是否已补全 */
export const user_is_complete = (userId) => {
    return AxiosCore.request({url: API_URL.userIsComplete + userId, method: 'GET', isAuth: false})
}
/** 获取unionid之后判断用户是否注册 */
...
export const change_robot_name = (urlComple, parmas) => {
    return AxiosCore.request({url: API_URL.saveWelcome + urlComple, data: parmas, method: 'POST', isAuth: true})
}
